...
model_service_key = os.getenv("MODEL_ENDPOINT_SECRET")
    ...
    while not ready:
        try:
            ...
            if model_service_key is not None:
                headers = {"Authorization": f"Bearer {model_service_key}"}
            request_cpp = get_request(f"{model_service}/models", headers)
            request_ollama = get_request(f"{model_service[:-2]}api/tags", headers)
